...
def analyze():
    ...
    # ── Detect label columns vs value columns ────────────────────────────────
    # Named label cols (label, label_1 …)
    named_label_cols = [c for c in funds_df.columns if 'label' in c.lower()]
    # Also treat any col_N column that is predominantly text as a label column.
    # This handles AmeriCredit / GM Financial / Drive Auto / Santander where the
    # label text lands in a generic col_N column instead of a label_N column.
    all_col_n = [c for c in funds_df.columns
                 if c.startswith('col_') and c not in named_label_cols]
    extra_label_cols = []
    for c in all_col_n:
        sample = funds_df[c].dropna().astype(str)
        if sample.empty:
            continue
        # If fewer than 20% of non-empty values parse as numbers → text column
        numeric_frac = pd.to_numeric(
            sample.str.replace(r'[$,\s()\-]', '', regex=True), errors='coerce'
        ).notna().mean()
        if numeric_frac < 0.20:
            extra_label_cols.append(c)

    label_cols = named_label_cols + extra_label_cols
    cols_to_check = [c for c in funds_df.columns
                     if c not in (['accession_number', 'label_str'] + label_cols)]
    print(f"  Aggregating numeric data from {len(cols_to_check)} columns...")
    print(f"  Label columns (named + text col_N): {len(label_cols)}")

    # Fast vectorized cleaning of all value columns
    ...
    print(f"  Checking {len(label_cols)} label columns: {label_cols[:10]}{'...' if len(label_cols)>10 else ''}")
    
    funds_df['is_prio_1'] = False
    funds_df['is_prio_2'] = False
    
    # ── Priority 1: Specific "total collections" labels ─────────────────────────
    # Organized by issuer for maintainability:
    p1_regex = (
        # Generic / multi-issuer
        r'^Collections$'
        r'|Total Collections'
        r'|Available Collections'
        r'|Available Finance Charge Collections'
        r'|Collections on Receivables'
        r'|Total Finance Charge and Principal'
        r'|Total Available Collections'
        r'|Amount Available for Deposit'         # Toyota
        r'|Total Available for Distribution'     # Hyundai, Nissan
        r'|Yield Supplement Account Deposit'     # Honda
        r'|a\. Collections'                      # Capital One
        r'|Principal Collections on Receivables during period'  # GM Financial
        r'|Interest Collections on Receivables during period'   # GM Financial
        r'|RECONCILIATION OF COLLECTION ACCOUNT' # Drive Auto, Santander
        r'|C\. COLLECTIONS AND AVAILABLE FUNDS'  # Volkswagen
        r'|C \. COLLECTIONS AND AVAILABLE FUNDS' # Volkswagen (alternate spacing)
        r'|Total Available Funds'                # AmeriCredit, GM Financial, Drive Auto, Santander, BMW
        r'|Interest Collected on Receivables'    # BMW, Capital One
        r'|Principal Collection on Receivables'  # BMW
    )

    # ── Priority 2: Broader fallbacks when P1 has no match ───────────────────
    p2_regex = (
        r'Available Funds'
        r'|Total Available'
        ...
        r'|Available Amounts'
        r'|Available Funds:'                     # BMW, Capital One, Exeter, AmeriCredit colon variant
        r'|Total Available Interest'             # BMW
        r'|Total Available Principal'            # BMW
        r'|Collection Account'                   # Capital One fallback
    )
    ...
    # ── 3.5. INTEGRATE REPAIRED DATA (The Repair Engine results) ──────────────
    # This recovers the Mercedes, Toyota, and Honda data we manually re-scraped
    if os.path.exists('output/repaired_collections_full.csv'):
        print("  Integrating results from Repair Engine...")
        repaired_df = pd.read_csv('output/repaired_collections_full.csv')
        repaired_df = repaired_df[repaired_df['scraped_total_collections'] > 0]
        
        # Create a dictionary for quick lookup by accession
        repair_map = repaired_df.set_index('accession_number')['scraped_total_collections'].to_dict()
        
        # Apply repairs to the unified summary
        def apply_repair(row):
            acc = row['accession_number']
            if acc in repair_map:
                return repair_map[acc]
            return row['scraped_total_collections']
            
        unified_summary['scraped_total_collections'] = unified_summary.apply(apply_repair, axis=1)
        
        # Also add accessions that were completely missing from the original CSV 
        # but are present in the repaired CSV
        missing_accs = set(repaired_df['accession_number']) - set(unified_summary['accession_number'])
        if missing_accs:
            new_rows = repaired_df[repaired_df['accession_number'].isin(missing_accs)]
            # Match with metadata to get report_period/company_name
            new_rows = pd.merge(new_rows, metadata_df, on='accession_number', suffixes=('', '_meta'))
            # Standardize columns
            if 'scraped_total_collections' in new_rows.columns:
                unified_summary = pd.concat([unified_summary, new_rows], ignore_index=True)
                print(f"    Added {len(new_rows)} missing filings from repair engine.")
    ...
    print(f"  Final Scraped dataset: {len(unified_summary)} filings, {unified_summary['company_name'].nunique()} pools")
    ...
    print("Joining datasets (Fuzzy Issuer/Year-Month)...")
    
    # ── FUZZY JOIN HACK ──
    # Create join keys that are just (Issuer_Base, Year-Month)
    provided_df['join_key'] = provided_df['issuer'].str.split().str[0].str.upper() + "_" + provided_df['year_month']
    unified_summary['join_key'] = unified_summary['parent_issuer'].str.split().str[0].str.upper() + "_" + unified_summary['year_month']
    
    final_df = pd.merge(
        provided_df, unified_summary,
        on=['join_key', 'year_month'],
        how='inner'
    )
    
    if final_df.empty:
        # Fallback to the original join if the fuzzy one is too narrow
        print("  Fuzzy join failed. Falling back to clean-name join.")
        final_df = pd.merge(
            provided_df, unified_summary,
